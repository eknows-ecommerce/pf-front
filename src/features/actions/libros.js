  ...
  try {
    const { data } = await axios.get(
      Production === 'production'
        ? `https://e-knows-back.herokuapp.com/libros?${query}`
...
export const getById = createAsyncThunk('libros/@GETBYID', async (id) => {
  try {
    const { data } = await axios.get(
      Production === 'production'
        ? `https://e-knows-back.herokuapp.com/libros/${id}`
...
export const create = createAsyncThunk('libros/@CREATE', async (libro) => {
  try {
    const { data } = await axios.post(
      Production === 'production'
        ? `https://e-knows-back.herokuapp.com/libros`
...
export const update = createAsyncThunk('libros/@UPDATE', async (libro) => {
  try {
    const { data } = await axios.put(
      Production === 'production'
        ? `https://e-knows-back.herokuapp.com/libros/${libro.id}`
