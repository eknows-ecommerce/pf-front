  ...
  const handleSubmit = async (e) => {
    ...
    if (validarInputText(valorNuevoItem)) {
      if (!image) {
        ...
        editarItem(res.data.secure_url)
      }
    } else {
  ...
  return (
    <>
      <div id="popup" className="z-50 fixed w-full flex justify-center inset-0">
        <div
          onClick={() => setEditarItemModal(false)}
          ...
          <div className="flex items-center justify-center h-full w-full">
            <div className="bg-white rounded-md shadow fixed overflow-y-auto sm:h-auto w-10/12 md:w-8/12 lg:w-1/2 2xl:w-2/5">
              <div className="bg-gray-100 rounded-tl-md rounded-tr-md px-4 md:px-8 md:py-4 py-7 flex items-center justify-between">
                <p className="text-base font-semibold">Editar {tipo}</p>
              </div>
              <div
                className={`px-4 md:px-10 md:pb-4 pb-7 ${
                  tipo === 'categoria' && 'pt-6 md:pt-12'
                }`}
              >
                {tipo === 'categoria' && (
